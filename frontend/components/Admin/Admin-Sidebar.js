...
const Sidebar = () => {
    ...
    const navItems = [
        { to: '/admin/dashboard', icon: <HomeIcon />, label: 'Home' },
        {
            ...
            icon: <StoreIcon />,
            children: [
                { to: '/admin/Add-Product', label: 'Add Product', icon: <AddProductIcon /> }
            ]
        },
        {
            label: 'Categories',
            icon: <StoreIcon />,
            children: [
                { to: '/admin/Add-Catagories', label: 'Add Categories', icon: <AddCategoriesIcon /> },
                { to: '/admin/Sub-Catagories', label: 'Sub Categories', icon: <SubCategoriesIcon /> }
            ]
        },
                            ...
                            {item.children ? (
                                <div>
                                    <div
                                        ...
                                        onClick={item.label === 'Product' ? toggleProductDropdown : toggleCategoriesDropdown}
                                    >
                                        <div className='mx-auto md:mx-0 md:truncate md:flex gap-2'>
                                            <div className="shrink-0">{item.icon}</div>
                                            <span className="truncate hidden md:block">{item.label}</span>
                                            <div className={`ml-auto transition-transform ${item.label === 'Product' ? (productDropdownOpen ? 'rotate-180' : '') : (categoriesDropdownOpen ? 'rotate-180' : '')}`}><ChevronIcon /></div>
                                        </div>
                                    </div>
                                                ...
                                                <Link key={childIndex} href={child.to}>
                                                    <div className="md:p-3 bg-gray-100 rounded-sm border shadow-sm font-semibold text-black transition-all flex gap-2 cursor-pointer">
                                                        <div className='mx-auto md:mx-0 md:truncate md:flex gap-2'>
                                                            ...
                                                            <span className="truncate hidden md:block">{child.label}</span>
                                                        </div>
                                                    </div>
                                                </Link>
                                            ))}
