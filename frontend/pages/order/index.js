            ...
            <thead className="bg-gray-200 text-gray-700">
              <tr>
                ...
                <th className="py-3 px-4 border-b text-left">Total Amount</th>
                 <th className="py-3 px-4 border-b text-left">Delivery Status</th>
            ...
            <tbody className="text-gray-600">
              {filteredOrders.map((order) => (
                <tr key={order._id} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="py-4 px-4 border-b">${order.totalAmount.toFixed(2)}</td>
                   <td className="py-4 px-4 border-b">{order.deliveryStatus}</td>
                  <td className="py-4 px-4 border-b">{order.shippingAddress}</td>
                    <td className="py-4 px-4 border-b">
                    <ul className="list-none p-0 m-0">
                      {order.orderItems.map((item, index) => (
                        <li key={index} className="border-b py-2">
                          <p className="text-sm font-semibold">Item ID: {item._id}</p>
